...
{ 
  ...
  function start_lvl( &$output, $depth = 0, $args = array() ) 
  {
    ...
    if ($depth > 0){
      $output	.= "\n$indent<ul class=\"mega-dropdown-lists\">\n";
    }
    else {
      $output	.= "\n$indent<div class=\"dropdown-menu mega-dropdown-menu\" aria-labelledby=\"navbarDropdown\">\n$indent<ul>\n";
    }
  ...
  function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) 
  {
    ...
    $attributes .= ($args->has_children || $hasMegaMenu) ? ' class="dropdown-toggle nav-link" data-toggle="dropdown"' : 'class="nav-link"'; 
    $fl_attributes = ! empty( $item->url ) ? ' href="' . esc_attr( $item->url ) .'"' : '';
    $fl_attributes .=  'class="nav-link"';
     
    $item_output = $args->before;
    if ( $depth == 1 ) {
      $item_output .= '<h3><a'. $fl_attributes .'>';
    }
    else {
